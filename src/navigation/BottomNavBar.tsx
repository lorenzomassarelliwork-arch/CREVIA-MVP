...
export default function BottomNavBar({ state, descriptors, navigation, position, layout }: MaterialTopTabBarProps) {
  ...
  const tabWidth = (layout.width || SCREEN_WIDTH) / Math.max(state.routes.length, 1);
  const inputRange = state.routes.map((_, index) => index);
  const indicatorTranslation = position.interpolate({ inputRange, outputRange: inputRange.map((index) => index * tabWidth), extrapolate: 'clamp' });

  return (
    <View style={styles.navBar}>
      <Animated.View style={[styles.indicator, { width: tabWidth, transform: [{ translateX: indicatorTranslation }] }]} />
      {state.routes.map((route, index) => {
        const routeName = route.name as TabRouteName;
        const isFocused = state.index === index;
        const options = descriptors[route.key]?.options;
        const { icon, iconOutline } = TAB_CONFIG[routeName];
        return (
          <TouchableOpacity
            key={route.key}
          ...
          >
            <Ionicons name={isFocused ? icon : iconOutline} size={24} color={isFocused ? colors.primary : colors.gray} />
            <Text style={[styles.navText, isFocused && styles.navTextActive]}>{labels[routeName]}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
