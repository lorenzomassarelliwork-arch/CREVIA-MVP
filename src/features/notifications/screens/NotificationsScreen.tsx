...
export default function NotificationsScreen({ navigation }: Props) {
  ...
  const grouped = visible.reduce<Record<string, AppNotification[]>>(
    (acc, item) => {
  ...
  const openNotification = async (item: AppNotification) => {
    ...
    if (
      item.type === 'experience_pending' ||
      item.type === 'experience_verified'
    ) {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}>
          ...
          <Text style={styles.headerSub}>
            {unreadCount > 0 ? `${unreadCount} da leggere` : 'Sei aggiornato'}
          </Text>
        </View>
        <TouchableOpacity
        ...
        >
          <Ionicons
            ...
            size={20}
            color={unreadCount > 0 ? colors.primary : colors.disabled}
          />
        </TouchableOpacity>
      </View>
          ...
          {visible.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyTitle}>
                {filter === 'unread' ? 'Tutto letto' : 'Nessuna notifica'}
              </Text>
              <Text style={styles.emptyText}>
                {filter === 'unread'
                  ? 'Non hai notifiche da leggere.'
                  : 'Qui vedrai candidature, aggiornamenti sui progetti e Crevia Experience.'}
              </Text>
            </View>
          ) : (
