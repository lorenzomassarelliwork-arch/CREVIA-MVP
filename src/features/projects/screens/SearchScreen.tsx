...
export default function SearchScreen({ navigation }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setItems(results);
    setBuilders(
      profiles.filter((profile) => profile.id !== CURRENT_USER_ID)
    );
  }, []);
  ...
  const filteredProjects = items.filter(({ project, roles }) => {
    ...
    return [
      ...
      project.category,
      project.city ?? '',
      ...roles.flatMap((role) => [role.title, ...role.requiredSkills]),
    ].some((value) => value.toLowerCase().includes(normalizedQuery));
  });

  const filteredBuilders = builders.filter((profile) => {
    if (!normalizedQuery) return true;
  ...
  const hasNoResults =
    filteredProjects.length === 0 && filteredBuilders.length === 0;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchBox}>
        ...
        <TextInput
          ...
          placeholder="Progetto, ruolo, skill, builder, città..."
      ...
      >
        ...
        {filteredProjects.map(({ project, roles }) => (
          <TouchableOpacity
            key={project.id}
            style={styles.card}
            activeOpacity={0.78}
            ...
            <View style={styles.cardTop}>
              ...
              <View style={styles.flex}>
                ...
                <Text style={styles.meta}>
                  {project.category} · {project.city ?? 'Remoto'} ·{' '}
            ...
            <View style={styles.chips}>
              {roles.slice(0, 3).map((role) => (
                <View key={role.id} style={styles.chip}>
                  <Text style={styles.chipText}>{role.title}</Text>
                </View>
              ))}
            </View>
          </TouchableOpacity>
        ...
        <Text style={styles.count}>
          ...
          {filteredBuilders.length === 1 ? 'builder' : 'builder'}
        </Text>

        {filteredBuilders.map((profile) => (
          <TouchableOpacity
            key={profile.id}
          ...
          >
            ...
            <View style={styles.flex}>
              <Text style={styles.title}>{getProfileDisplayName(profile)}</Text>
              <Text style={styles.meta}>
                {profile.headline ?? 'Builder'} ·{' '}
                {profile.city ?? 'Località non indicata'}
              </Text>
        ...
        {hasNoResults ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>
              Prova con un progetto, un ruolo, una competenza o un builder
              diverso.
            </Text>
          </View>
        ) : null}
...
const makeStyles = (c: ColorPalette, top: number, bottom: number) =>
  StyleSheet.create({
    ...
    count: { fontSize: 12, fontWeight: '700', color: c.gray },
    card: {
      padding: 16,
      gap: 11,
      borderRadius: 16,
    ...
    description: { fontSize: 13, lineHeight: 19, color: c.textMuted },
